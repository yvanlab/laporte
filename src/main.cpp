...
SensorManager sensorMger(pinLed);
...
void loop(void)
{
  ...
  if (sensorMger.isActivated())
  {
    DEBUGLOG("sensorMger.isActivated");
    ...
    if (!fpManager.isActivated())
    {
      ...
      DEBUGLOG("NOT fpManager.isActivated()");
    }
    fpManager.activateModule(true);
  }
  ...
  if (fpManager.isActivated())
  {
    DEBUGLOG("fpManager.isActivated");
  ...
  if (mtTimer.is1HPeriod())
  {
    if (WiFi.isConnected())
    {
      grovesMgt.addVariable(PRESENCE_COULOIR, String(smManager.nbCouloirDetection));
      grovesMgt.addVariable(PRESENCE_DRESSING, String(smManager.nbDressingDoor));
